...
class EqualLinear(layers.Layer):
    ...
    def build(self, input_shape):
        # Ensure input_shape is a list or tuple, and get the last dimension
...
class ConstantInput(layers.Layer):
    ...
    def call(self, inputs):
        batch = tf.shape(inputs)[0]
        # Access the input_tensor correctly, not input
...
class Generator(keras.Model):
    def __init__(
        ...
        # Define log_size (based on the input size)
        self.log_size = int(np.log2(size))

        # Channel configuration for different sizes
        self.channels = {
            4: 512,
            8: 512,
            16: 512,
        ...
        # Modify to use ResNet-based style convolutions (adjust as needed)
        self.resnet_layers = []

        for i in range(3, self.log_size + 1):
            out_channel = self.channels[2 ** i]
            # Modify to use ResNet convolutions instead of styled convolutions
            ...
            # Add residual connections if using ResNet
            self.resnet_layers.append(
                layers.Add()
            )
